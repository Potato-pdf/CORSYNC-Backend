...
// Al arrancar el simulador se elige un aura distinta (entre las seis) y nunca
// la misma dos veces seguidas, para que cada puesta en marcha cambie de color.
let lastRandomAura = null;
function randomScenario() {
  let pick;
  do { pick = AURA_ORDER[Math.floor(Math.random() * AURA_ORDER.length)]; }
  while (pick === lastRandomAura && AURA_ORDER.length > 1);
  lastRandomAura = pick;
  return pick;
}
...
const state = {
  ...
  scenario: null,               // clave de SCENARIOS | 'ciclo' | 'deriva' (se elige al azar al arrancar)
  sent: 0,
  lastReading: null,
...
function currentTarget() {
  if (!state.scenario) state.scenario = randomScenario();
...
server.listen(UI_PORT, () => {
  ...
  log(`Escenario inicial al azar → ${state.scenario}`);
  console.log('');
